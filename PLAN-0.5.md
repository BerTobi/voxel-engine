...
| **M4** | **Radial shell-snap finisher (NEW machinery) + cross-chunk flow + pin.** Replace the Y-column solver with the **radial-bin connected-body solver** (§4.4), gated by `VOXEL_FLUID_SNAP_MAX` + an off-switch over M3's proven baseline; re-derive the sleep guard radially with the anti-re-fire guard; open water across chunk faces over the `sim_tick_ex` seam; new `world_pin`/`world_unpin` against the 1777-slot pool. | `test_water_seam.c` + `test_finisher.c`: a **multi-chunk** pour settles to `active==0` as one connected radial shell, conserved across seams; the confirm window does not re-fire on the X/Z limit cycle; the snap **with the off-switch on** still ships a conserving (terraced) lake; a pinned active water chunk is not evicted mid-flow and 48·7 pins do not starve streaming; light re-bakes when water carves a cave. | **YES** (cross-chunk flow, finisher, and pin are additive; save/wire formats untouched until M5) |
| **M5** | **Host-authoritative water-delta streaming.** Stream `CHUNK_MODIFIED_BY_SIM` water chunks via `MSG_CDELTA` (clients run zero CA), **RLE** the wire payload. Bump `NET_PROTOCOL_VERSION 3→4`. Wire-budget gate (≤24576 B < `NET_CHUNK_MAX`, single-frame, no per-voxel flow byte). | `test_water_net.c`: two-peer **water render-fidelity** via the GL-free driver; ≥4 simultaneous springs converge under the per-frame chunk budget; CDELTA fragmentation (delta > `NET_CHUNK_MAX` split/reassembled); handshake refuses a v3 peer; bandwidth/CPU probe. | **YES** (net handshake gated cleanly; save format unchanged from M2) |
